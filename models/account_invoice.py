...
class AccountInvoice(models.Model):
    ...
    discount_type = fields.Selection(
        [('nodiscount', 'No Discount'),('percent', 'Percentage'), ('amount', 'Amount')],
        string='Discount type',
        readonly=True,
        states={'draft': [('readonly', True)], 'posted': [('readonly', True)]},
        default='percent')
    discount_rate = fields.Float('Discount Rate', digits=(16, 2),
        readonly=True,
        states={'draft': [('readonly', True)], 'posted': [('readonly', True)]}
    )
    discount_head = fields.Char(string="Discount Head", states={'draft': [('readonly', True)], 'posted': [('readonly', True)]})
    amount_discount = fields.Monetary(string='Discount', store=True, compute='_compute_amount', readonly=True, tracking=True)
